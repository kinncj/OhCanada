...
| `TN-READ-reading-a-passage-at-a-stop.md` | `TN-READ` | Reading a passage of the guide at a stop in a level: how the words get to the screen, and the single-switch and screen-reader routes through prose (ADR-0063) |
| `TN-LEARN-reading-the-guide-by-chapter.md` | `TN-LEARN` | Learn: the guide's chapters in the guide's order, a chapter's lessons, the reader, Back at each step; every shippable passage reachable (ADR-0061, ADR-0065 §3.1) |
| `TN-TEACHBACK-read-about-this.md` | `TN-TEACHBACK` | "Read about this" after a question is judged — in a level, in Study and in the exam review, never on a live exam question (ADR-0070) |
...
| **ready to come back** | The scheduler would offer this question now. Never said on screen — the player sees only "New question" or "You have seen this question before" (ADR-0036; the rows were "New" and "Seen before"). |
...
| `character-creator`, `character-preview`, `character-preview-art`, `randomise-character`, `start-playing`, `creator-done`, `creator-back`, `creator-settings`, `creator-save-error`, `creator-retry`, `creator-continue`, `creator-option-gone` | The character creator (`TN-CREATOR`, `TN-FIRSTRUN`). `start-playing` and `creator-done` are never both present: the first is drawn on a first run and the second when the screen was opened from Settings. `character-preview` is named by `creator.preview.label` and reports `data-skin`, `data-hair-shape`, `data-hair-colour`, `data-head-covering` and `data-feature`. `character-preview-art` is the `aria-hidden` picture inside it (ADR-0040, `TN-CREATOR-12`) and reports `data-state` (`loading`, `ready`, `failed`) and `data-frames`, the atlas frames drawn, back to front. |
...
| `poi-card-art`, `dialogue-portrait`, `quest-complete-stamp-art`, `passport-stamp-art-<levelId>`, `title-art`, `title-landscape`, `title-figure`, `study-art` | Decorative pictures (ADR-0041; the passport's stamps, ADR-0045): an `aria-hidden` frame holding an `alt=""` image, or the stamp's ring and silhouette. Each publishes `data-state` (`empty`, `loading`, `ready`, `failed`) and hides itself when its picture cannot load; the stamp also publishes `data-inked`. None is ever the only place a fact is drawn. |
| `poi-card`, `poi-card-close` | Landmark information card. One of the two places a name on `TN-NAMES`'s list may appear. |
| `lesson-reader`, `lesson-reader-title`, `lesson-reader-body`, `lesson-reader-passage`, `lesson-reader-close` | The lesson reader, opened by a quest step of kind `read` (`TN-READ`, ADR-0063). `lesson-reader-title` is the lesson's own title and the dialog's accessible name; `lesson-reader-body` is the prose and the dialog's accessible **description**, and carries no role and no `tabindex`, so nothing stands between a reading cursor and the words. Each `lesson-reader-passage` is one authored passage, reported by `data-tn-passage`: it is a **switch stop** and is out of the Tab order, so one short press moves to it, scrolls it into view and reads it aloud, and a long press reads it again and does nothing else. No passage text is ever written in a quest or a level document — the step names `{ lesson, passage }` pairs and the words stay in `content/lessons/**`. |
| `title-learn`, `learn`, `learn-title`, `learn-intro`, `learn-chapters`, `learn-chapter-<directory>`, `learn-lessons`, `learn-lesson-<lesson id>`, `learn-back`, `learn-retry`, `learn-error`, `learn-empty` | Learn (`TN-LEARN`, ADR-0061). `learn` is a dialog named by `learn-title` ("Learn", then the open chapter's name); the two lists are ordered lists of buttons in the guide's order and the authored lesson order. A lesson opens `TN-READ`'s `lesson-reader`, unchanged. Back and Escape go one step up. `learn-error` offers `learn-retry`; `learn-empty` deliberately does not. |
| `question-read-about`, `exam-review-read-about-<n>` | "Read about this" (`TN-TEACHBACK`, ADR-0070): on the question card once an answer is judged, and on review item `<n>` of the exam review. Drawn only when a readable lesson passage shares the question's proposition — never disabled, never on a live exam question. Opens `lesson-reader`; closing it returns focus here. |
...
| `question-card`, `question-place`, `question-kind`, `question-progress`, `question-prompt`, `option-0`…`option-3`, `question-feedback`, `question-explanation`, `question-next`, `question-close`, `question-closed-notice` | The question card. `question-place` names the level, and the landmark whose card has just been read, and is hidden in Study (ADR-0045). In an exam, `question-kind`, `question-feedback` and `question-explanation` are absent (`TN-EXAM-03`). |
...
| `storage-warning`, `save-error`, `save-export`, `save-import`, `save-import-file`, `save-import-error`, `save-import-confirm`, `save-import-replace`, `save-import-keep`, `save-import-done`, `save-import-continue`, `save-clear`, `save-clear-confirm`, `save-clear-yes`, `save-clear-keep`, `save-clear-error` | Persistence. `storage-warning` is owned by `TN-HUD-03` and appears on the title screen too (`OQ-HUD-3`). `save-import-file` is the hidden native input the button opens and is never a Tab stop; `save-import-error` carries a refused **file** and `save-clear-error` a delete that did not finish, so a screen reader coming back to either control hears why that control's last try failed. `save-clear` is the one destructive control in the game (`TN-SAVE-06`): it asks `save.clear.confirm` and, on yes, clears storage and opens the title screen as it is for a first-time player, so there is no dialog after it. |
