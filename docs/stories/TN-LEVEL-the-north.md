...
when it fails and when it is finished — in both languages, with no vessel and no nation named on a screen
that may not name one, with a territorial statement quoted from Kwanlin Dün First Nation left whole and in
one place, and with the article in lower case in every sentence that is not a title.

**This is not the whole level story.** `TN-LEVELS-2-to-10-spine.md` fixes what this level is; the full story
— the walk along the bar, the sternwheeler's card, an NPC if one is ever placed, a quest if one is ever
authored — is written in the slice that builds them, in the shape of `TN-LEVEL-ottawa.md`. What is here is
the copy the level draws **today**, because `content/levels/the-north.json` shipped,
...
**This is the tenth level, and the map is now complete: ten built, no empty slots.** That changes one thing
...
1. **No copy on this level makes its subject a territory.** Not the title, not the subject line, not the
   waiting sentence, not the error title, not the stamp sentence, not the play label (`TN-NORTH-06`).
2. **The territorial statement lives in "About this place" and nowhere else** (`docs/content-review.md`
   ...
   player taps past; a compressed paraphrase of a cited sentence is an unsourced claim about a nation.
3. **Nothing in this level depicts anybody.** `assets/style/the-north-level.md` §0 records `neverAdd` on
   ...
   *Discover Canada*** — so a figure drawn on a northern river bank is read as somebody, and this project has
   ...
   this level describes, addresses or names a person, a people or a nation.
...
- **The Ta'an Kwäch'än Council is not named, in either direction.** Its government is in Whitehorse and about
  ...
  this file resolves that silence**, and none of them says "the nation", "the only nation" or "whose land
  this is" either — a sentence explaining the silence would be the same claim with an apology attached.
  This is the same shape as level 8's silence about the Métis Nation of Alberta (`TN-ALBERTA`).
...
either language, and `communityReview.status` is never rendered on any screen (`TN-NORTH-06`).

**The marker is currently checked by nothing** — `scripts/check-obligations.mjs` scans `*.md` under `docs/`
only. It is copied into `docs/content-review.md` §13 in the same change as this file. See `OQ-NORTH-5`.
...
that moves a willow clump or a piece of driftwood.
...
with nothing in either the title table or the waiting table, and needs no gloss.

**It names no landmark and no territory.** The sternwheeler is named in the point-of-interest card's body and
**nowhere else** (`TN-NAMES-01`, `TN-REACH`); the Yukon River, Whitehorse and the three territories are named
in the level document's own sourced text and in no copy row in this file.
...
and five others appear in no string on this level in either language.

## What this level does not have yet

`content/levels/the-north.json` declares `"characters": []` and `"quests": []`. There is no NPC, no dialogue
and no task on this level today, so **the only way to finish it is to reach the end of it**, which earns the
stamp and draws the completion card (`TN-DONE`). Everything on that card has to be true of a player who
walked from the spawn to the exit. **Six of the ten built levels now declare no quest**, which is what
`OQ-DONE-1` is about and is now the majority of the game.
...
against `CLAUDE.md`'s thirty — behind `history`'s ninety-six and ahead of the other eight. It is also a
correction to `OQ-SPINE-3`, which recorded `regions` as having **no bank at all**, and to the spine's own
table, which said the same. Both are amended in this change rather than left to be re-derived.
`ADR-0028` is why the bank looks the way it does: the industry and resource material in *Canada's Regions*
belongs to `economy`, and what is left to this subject is the five regions, the provinces and territories,
their capitals, population distribution, languages and landforms.

## Accessibility and bilingual coverage map

| Path | Discharged by |
|---|---|
| Keyboard only | `TN-NORTH-03`; the escape route and the error buttons are `TN-WAIT-04`; the completion card is `TN-DONE-06` |
| Single switch | `TN-NORTH-03`; `TN-WAIT-04`; `TN-DONE-06` |
| Screen reader | `TN-NORTH-03`; `TN-DONE-07` |
...
| Depiction | `TN-NORTH-06`, which is on the same footing as the five above |
...
Feature: The North says what it is getting ready
  ...
  Scenario: The landmark is named as a type, because the vessel ships nameless
    ...
    Then "poi-card" shows "Yukon River sternwheeler" as text, inside a sentence that says what it is
  ...
  Scenario: The HUD does not name it either
    ...
    And no string drawn inside "hud" contains "sternwheeler" or "Yukon" in either language
```
...
Feature: The level in French
  ...
  Scenario: The landmark's French name is the one the level document carries
    Given the North level is playable
    When I engage the landmark
    Then "poi-card" shows "Vapeur à roue arrière du Yukon"
...
## TN-NORTH-05 — Finishing the North, and the end of the journey
...
Feature: The place is a place, the subject is a chapter, and no string says otherwise
  ...
  Scenario: One region is drawn and the copy claims no more than that
    Then no string in this file claims the level shows Nunavut or the Northwest Territories
    And no string claims the level shows the whole North
    And the only screen naming the three territories is the point-of-interest card
    And that sentence is the level document's own text, sourced from Discover Canada
...
- **`OQ-NORTH-1` — an NPC, an arrival announcement and a per-target prompt are not written here, on purpose.**
  The shipped document places no character and declares no quest, so a speaker's label or an arrival sentence
  written today would be copy for behaviour nothing performs (ADR-0008). The landmark draws `TN-REACH`'s
  generic "Look at this place" / « Regarder ce lieu ». *Recommendation:* choose an **epicene** French role
  noun when this level's full story is written, so `OQ-LEVEL-8`'s agreement question never arises — and read
  `assets/style/the-north-level.md` §0 and §11 first, because a character placed on this level is the first
  figure this level has ever drawn and §1's shipping rule is engaged the moment one carries a marker.
- **`OQ-NORTH-2` — the level's facts are unverified.** `content/levels/the-north.json` carries
  `verification.status: "unverified"` on the point of interest's fact, on the territorial statement **and** on
  the `nationSource`, and `TN-LEVELS-03` requires every factual sentence a level puts on screen to be verified
  like a question. Nothing in this file's four rows states a fact, so none of them is blocked; the card and the
  panel are. **One thing a verifier may not do is recorded here so it is not discovered mid-review**: the
  statement rests on **three passages of one page** and a `FactSource` carries one quote, so two of the three
  are registered in `content/sources/kdfn-about-us.json` rather than quoted — and a verifier may neither
  collapse them into one quote nor resolve the Ta'an Kwäch'än silence. **ADR-0028 carries a second, separate
  obligation on this chapter** — a live check of *Canada's Regions* against canada.ca — which is not this
  file's and is worth knowing about by whoever picks this up. Routed to the content verifier.
- **`OQ-NORTH-3` — the level is called The North and is entirely the Yukon, and the copy carries that
  ...
  does not reopen it** — "The North" is the region *Discover Canada* teaches and the chapter the bank is
  keyed to. *Recommendation:* if a future slice gives Nunavut and the Northwest Territories their own levels,
  this level keeps its title and gains a place name in its point of interest; nothing in these four rows
  changes, which is the test of whether they were written at the right altitude. Routed to the plan owner.
- **`OQ-NORTH-4` — "the gravel shore" is the tenth waiting sentence and the checking is still by hand.**
  ...
  languages, and named "The North" / « Le Nord » as the one still to come most likely to collide with a plain
  noun. **It did not collide**, because the sentence names the stones rather than the direction — but that was
  established by reading ten titles and ten sentences, which is exactly what `OQ-WAIT-4` said should stop
  being a human's job. *Recommendation:* build the check now that all ten exist, because the set is complete
  for the first time and a complete set is the cheapest moment to write a comparison over it. Routed with
  `OQ-WAIT-2`.
- **`OQ-NORTH-5` — two Tier 3 obligations were written where nothing reads them, and copying them is a
  ...
  neither had a clock. Both are copied into `docs/content-review.md` §13 in this change.
  *Recommendation:* **widen the scanner to `assets/style/` as well**, and keep the copies. The copy is what
  makes the date real today; the widening is what stops the next art sheet writing a marker nothing reads.
  The art agent reported this plainly rather than assuming the marker counted, which is the behaviour that
  made it fixable in one commit. Routed to the engine agent; `scripts/` is not this directory's to edit.
  Same question as `OQ-PEGGYS-4`.
