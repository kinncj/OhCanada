...
`TN-LEVEL-ottawa.md` owns Ottawa's two per-target rows and the officer; `TN-NAMES-naming-real-places.md`
owns which names may appear where; `TN-COPY-strings-and-counts.md` owns the rules every row here obeys.
...
2. **It is a trade name on a screen that may not carry one.** `TN-NAMES-01` puts a name from its list in a
   point-of-interest card's **body** and nowhere else, and `TN-NAMES-04` fails the build for "a copy string
   drawn by the passport, the level select, the HUD or a menu" containing such a name. The prompt is drawn in
   the HUD. The name is not a copy string today — it is content interpolated at runtime — which is precisely
   how it got past a check written against copy tables.

Writing the rows below fixes both at once: the prompt stops being a name, so the HUD stops being a place a
name can leak into.

## The key shape, and which row wins

**Two kinds of row.** A generic row per kind, written once here, and an optional per-target row written in
the level's own story where a level has something better to say.

| Key | What it is |
|---|---|
| `hud.interact.poi` | A place is in reach and this level wrote nothing more specific |
| `hud.interact.npc` | A person is in reach and this level wrote nothing more specific |
| `hud.interact.done` | This one has already been done |
| `hud.interact.hint` | The one-time explanation of the marks themselves |
| `hud.interact.<target id>` | A level's own row for one target, written in that level's story file |

The target id is **the id the level document gives it** — `officer`, `parliament-hill`, `cn-tower` — because
that is what `poi/entered` carries and what the prompt is looked up by. Ottawa's two rows keep their wording
and are respelled to that shape (`OQ-REACH-1`).

**Which row is drawn, in order:**

1. **The target has been done** → `hud.interact.done`, whatever kind it is and whatever row the level wrote.
...
2. **The level's story writes a row for this target** → that row.
...
**Where a level can do better, it does.** Ottawa writes "Talk to the officer" and "Look at Parliament Hill",
because that level has a named character and a landmark that is not on `TN-NAMES`'s list. A level whose
landmark **is** on that list may not write such a row, and the generic one is the answer rather than a
worse-written specific one.
...
| `ready` | tapping it now will work | `hud.interact.npc`, `hud.interact.poi`, or the level's own row |
...
table can carry a name `TN-NAMES` keeps in a card body, and the HUD stops being a surface a name can reach.
...
## TN-REACH-02 — A level that has written no row of its own
...
Feature: The generic rows, and the names they keep out of the HUD
  ...
  Scenario: A place with no row of its own
    ...
    And no string drawn inside "hud" contains a name from TN-NAMES-naming-real-places.md's list
  ...
  Scenario: The name is where a name teaches something
    ...
    And that card is the only screen in this level that names it
    And the name is read to a screen reader there, not in the HUD
...
Feature: A mark that has already been used
  ...
  Scenario: Done wins over the level's own row and over the kind
    Given a level wrote its own row for this target
    Then the prompt is still "Done. See this one again"
    And the same is true for a person as for a place
...
Feature: The rule is checkable, not a promise
  ...
  Scenario: A name from the list drawn anywhere in the HUD fails the build
    Given a string drawn inside "hud" contains a name from TN-NAMES-naming-real-places.md's list
    When the content check runs
    Then the build fails, naming the string
    And the check reads what the HUD draws at runtime, not only what a copy table declares
...
Feature: The prompt reaches everybody
  ...
  Scenario: The announcement names a target only where a row names it
    Given the level's story wrote a row for this target
    Then the announcement contains that target's name
    Given the level's story wrote no row for it
    Then the announcement is the generic prompt and names nothing
    And no name from TN-NAMES-naming-real-places.md's list is ever announced outside "poi-card"
...
Feature: The prompt in French
  ...
  Scenario: A level's own row is French too
    Given the Ottawa level is playable
...
- **`OQ-REACH-4` — a blind player is not told which landmark is in reach where a level wrote no row.**
