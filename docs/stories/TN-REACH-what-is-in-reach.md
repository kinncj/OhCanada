...
**Measured 2026-09-21 on a 390 × 844 phone at 200 % text, where `system-ui` resolved to Liberation Sans:
31 of 144 real pairings put the player's own task below the HUD strip**, where they have to scroll to find
it. The face is named because it is load-bearing — see the end of this section. The mechanism is not character count — it is
...
**How many pairings fit after the pass is not a number this file can state, and that is the finding rather
than an evasion.** The rows are sized in `rem` and their `line-height` is fixed in the stylesheet, so the one
thing that varies between machines is glyph width — which is exactly what decides where a line wraps.
`system-ui` resolves to a different face on every platform, and on a face measurably wider than Liberation
Sans the same rows wrap a line earlier and the same tasks go under again. **The shortening below is worth
having on every face** — it is plainer copy and it removes a line wherever it is read — but it does not on
its own make the strip big enough, and no arrangement of these words does.

**A swept gate over every real pairing is owed, and so is a decision about the font.** Both are held outside
