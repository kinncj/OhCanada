...
full story — the bike, the square, the guide, the landmark card, the quest — is written in the slice that
...
tower.** `content/quests/toronto-cn-tower.json` ships, the level document places `characterId: "guide"`, and
there was no `npc.guide.name`, so the offer was refused; the name is now written in `TN-GUIDE-the-guide.md`.
The quest also **says "CN Tower"** — in its summary, in a line the guide speaks, and in a step prompt the
HUD's tracker draws — which `TN-DIALOGUE-what-a-quest-giver-says.md` rules is allowed and `TN-NAMES-01` now
says so. **One scenario in this file changed because of it**: the point-of-interest card is no longer the
only screen in this level that names the tower. What has **not** changed is the rule this file was written
for: the tower's name may not reach a loading screen, an error card, a stamp or `interact-prompt`, and the
last of those is still the one being broken today.
...
| What the guide says in this level's quest | authored `dialogue[].text`, `summary`, `steps[].prompt` | `content/quests/toronto-cn-tower.json`, under `TN-DIALOGUE-what-a-quest-giver-says.md` |
...
point-of-interest card's body or in a quest's own words about going there, naming a loading message as one of
the places it may not appear. The tower is named on the card the player rides up to, inside a sentence that
says what it is, with its source.

**Neither does the stamp sentence, and neither may the HUD's prompt.** A stamp is named after a place
...
tower reads **"CN Tower"** in `interact-prompt`, which `TN-NAMES-04` names among the surfaces that fail the
build. `TN-REACH-what-is-in-reach.md` is where that is fixed, and this level is its worked example. **The
quest tracker is the one string inside the HUD that may carry the name**, because it is this quest's own
step prompt and not chrome (`TN-DIALOGUE-06`).
...
composed.** Toronto, like Halifax, Ottawa and Winnipeg, takes no article in French — « charger Toronto »,
« le tampon de Toronto », « Jouer à Toronto » — and `TN-LEVEL-quebec-city.md` and
`TN-LEVEL-prairie-rail.md` are the rows in the same set that take one in all three. One template cannot be
right for all of them, so all six levels are written (`TN-WAIT`, `TN-DONE`).

## Accessibility and bilingual coverage map

| Path | Discharged by |
|---|---|
| Keyboard only | `TN-TORONTO-03`; the escape route and the error buttons are `TN-WAIT-04`; the completion card is `TN-DONE-06`; the quest's dialogue is `TN-GUIDE-04` |
| Single switch | `TN-TORONTO-03`; `TN-WAIT-04`; `TN-DONE-06`; `TN-GUIDE-04` |
| Screen reader | `TN-TORONTO-03`; `TN-DONE-07`; `TN-GUIDE-05` |
...
Feature: Toronto says what it is getting ready
  ...
  Scenario: This level's quest can be given
    ...
    And the event "quest/offered" is emitted for "toronto-cn-tower"
  ...
  Scenario: The tower is named where a name teaches something, and where the guide sends me
    Given the Toronto level is playable
...
Feature: The error card names this level
  Scenario: The assets cannot be fetched
    ...
    And it does not name Ottawa, Halifax, Québec City, Winnipeg or the Prairies
...
Feature: The level in French
  ...
  Scenario: The landmark's French name is the one the level document carries
    ...
    And this level's quest may contain it, in its own words
...
- **`OQ-TORONTO-1` — the arrival announcement is not written here, and the NPC question answered itself.**
  The announcement belongs with the square and the quest, in the full level story, for the reason
  `OQ-HALIFAX-1` gives. **The volunteer is gone**: this level places the guide, like levels 1 and 3, and
  `TN-LEVELS`'s NPC table is corrected. **The interact prompt is not waiting on any of that**: `TN-REACH`
  gives this level the generic row, and it may not write a per-target row for its landmark anyway, because
  the tower's name is on `TN-NAMES`'s list.
- **`OQ-TORONTO-2` — `TN-LEVELS` gives this level Toronto City Hall as its landmark and the shipped document
  ...
  anchor on the skyline; `content/levels/toronto.json` ships one point of interest and it is the tower, and
  the shipped quest is named after it and sends the player to it. Nothing in this file's four rows turns on
  it — none of them names either building — but the level's full story cannot be written until the two agree,
  and **the quest raises the cost of changing it**: a landmark swap now means rewriting authored dialogue,
  not just an art ticket. **It does change one thing**: Toronto City Hall is on `TN-NAMES`'s list as a civic
  building, so whichever wins, no prompt, stamp or loading sentence may name it. Routed to the plan owner and
  the art agent, with `OQ-SPINE-5`.
- ~~**`OQ-TORONTO-3` — level 5 and level 4 draw from one question bank.**~~ **Answered 2026-09-09.**
  `content/questions/elections/` now holds 36 verified questions of its own — the ballot, advance-poll,
  electoral-district and who-may-vote questions that were filed under `government` — and `government` keeps
  38. Both levels clear `CLAUDE.md`'s thirty from their own bank, which is what `OQ-SPINE-3` asked for, and
  this level's quest has a bank to draw from. Recorded as answered because this file carried the worry
  while the level was openable and empty.
