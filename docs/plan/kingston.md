...
| K-0.7 | **Boundary defect, still open after `map-insets`.** The anchor rule is now keyed on `journey`, and it fails both ways: a journey id with no anchor, and an anchor naming no journey place (`screen-art.mjs`, "names no place in game.config.json#/journey"). So Kingston's journey id (content) and its anchor (art) still have to land in one commit. The defect moved from the level document to the config. Fix: let an anchor precede its place, with the reverse check kept for ids that no story, level or anchor sheet declares. Or keep ADR-0069 §6's fallback: content lands both with art's published coordinates, and art reviews | infra (the fix) or content (the fallback) | `scripts/lib/screen-art.mjs`, `tests/unit/infra/…` | With the fix, K-2.2 carries no art file. **Done 2026-09-25, branch `kingston-infra` (the fix; ADR-0069, "Amendment, 2026-09-25"):** an anchor ahead of its journey slot passes when `docs/stories/TN-LEVEL-<id>.md` or a level document declares the place, and is reported; an undeclared id still fails. A journey place with no anchor is reported as AWAITING by `make validate-content` and fails under `validate-content --release`, which `make build` now runs, so CI (which builds every PR) and the deploy hold the old rule on the final tree. **For Kingston:** art's anchors (`anchors.kingston` (621.0, 526.6) and `insets[1].anchors.kingston` (895.2, 268.8)) can land in art's own commit, before or after K-2.2, and pass today on `TN-LEVEL-kingston.md`. K-2.2 carries no art file. If K-2.2 lands first, the PR is green only once art's commit is in it |
| K-0.8 | Data-driven blind hand-off. Québec City's stops needed `'wolfe-montcalm': singleSource()` added to `scripts/lib/art-handoff.mjs`, which is an infra file edited for an art subject. Read the composition from `assets/refs/references.json` instead | infra | `scripts/lib/art-handoff.mjs`, `tests/unit/infra/art-handoff-gate.test.ts` | A new subject needs no script edit. **Done 2026-09-25, branch `kingston-infra`:** a subject in `assets/refs/references.json` may carry `"handoff": { "builder": "single-source" }` or `{ "builder": "two-parallax-tiles", "farMatch", "nearMatch", "nearTop", "what" }` (`resolveBuilder`). The same builders run, so anonymisation, padding and the leak scan are unchanged. Refused: a subject stated both in `RECIPES` and in the contract, an unknown builder, an unknown, missing or mistyped parameter, two matches selecting one render, and `handoff` with `renders: []`. The existing `RECIPES` entries stay; moving one to the contract also deletes its `RECIPES` line, an infra edit, so it is optional and joint. Bespoke builders (figures, mounted subjects, comparisons) stay code. **For K-2.1:** Kingston's subjects declare `handoff` and touch no infra file |
| K-0.9 | Product owner rulings: the subtitles (`level.kingston.subtitle` "Building Canada", and Québec City narrowed to "Early Canada"), "of 11" in the exam and passport stories, OQ-KINGSTON-1 (the giver), OQ-KINGSTON-2 (`hist-39` at Kingston Mills), OQ-KINGSTON-3 (content-review §1 item 5) | po | `docs/stories/TN-LEVELS-2-to-10-spine.md`, `TN-LEVEL-kingston.md`, the exam and passport stories | ADR-0068's `owner=po` marker, due 2026-12-23 |
...
| K-3.2 | The territory statement's `about-this-place` checks and the Macdonald panel question (OQ-KINGSTON-4) stay with the product owner. No agent writes that panel | po | `docs/stories/TN-LEVEL-kingston.md` |
