...
| 1.23 | Player-facing shell: the screens made to look like a game, Study and the in-level learning loop mounted, settings reachable from the strip, and a finished level leading to the next | ui-a11y | Every DOM screen drawn from `assets/style/palette.json` through `app/ui/palette.ts`, with contrast measured rather than asserted; axe clean on every screen including the two new ones; Study runs a real drill on `dist/`; a landmark teaches then asks; earning a stamp lands the player on the card that opened | **Done, with three named gaps.** (1) `SceneLevel` carries no `subject`, so a landmark's question is drawn from the whole bank rather than from the level's subject (`TN-CARD-01` asks for the subject). (2) `SceneLevel` carries no `quests` and every level document declares `"quests": []`, so `answerQuestion` can never report `stampEarned` and the completion card is wired to a signal no content can raise — the card, the map handoff and their tests are complete and proved in the harness and in `tests/unit/bootstrap`. (3) Three copy rows are missing and were reported rather than invented: `hud.interact.*`, `stamp.<id>.earned` for every level but Ottawa, and a sentence naming the level that just opened. |
