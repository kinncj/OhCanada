...
The ledge nearest the viewer: bare rounded whaleback granite broken by joints into masses of very different sizes, a lit crest on each, grass and crowberry in the joints, yellow `felt-light` lichen, and two shallow irregular tide pools. It continues the jointed rock of `layer-40-granite-barrens`. No erratic: the erratic is a POI.

**Redrawn on 2026-09-17 against the played scene, for two reads a player got and the tile on its own did not give.** (1) *A paved street.* Every mass was a near-rectangle of near-identical width laid in straight rows, with the vertical joints of one row landing near those of the next and a dark band ruled under each — which is masonry coursing, and the bottom third of this level read as flagstone paving rather than as the bare granite Peggy's Point is. The masses now run four to a band at four different widths, no two alike; the band edges rise and fall rather than ruling straight across; three masses cross a band boundary so no continuous horizontal line survives; and the joints are the shade tone showing through from beneath rather than a line drawn on top. (2) *Manhole covers.* Each pool was concentric ellipses ringed in `brass-shade`, and a bright warm ring around a flat blue disc set in grey paving is a drain lid. Each pool is now an irregular eleven-point hollow, its long axis across the rock, with water in two flat tones, glints as two short bars, and rockweed as three or four broken dark tufts gathered on **one** side, so nothing encircles it — which is how `layer-40-granite-barrens` already drew its pools.
