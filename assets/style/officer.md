...
| 2 | **Dark collar and shoulder straps** | Midnight blue, distinctly darker than the tunic. The colour break at the collar and at the shoulders is what stops the tunic reading as a plain red coat. The collar sits at the **base of the neck** with its top edge scooped, not up under the jaw: it was drawn at the jaw, which left this character with no neck at all, and navy directly under lit skin is now the strongest neck read of the three costumes. See `art-bible.md` §7.3. | `navy` |
| 3 | **Brass buttons** | A single row down the centre front, plus one on each pocket flap. The row starts BELOW the collar; it moved down 16 px when the collar dropped to the base of the neck, and the lanyard's top moved with it. Read them as a rhythm of round brass dots, not as modelled buttons. | `brass` |
| 4 | **Wide-brimmed hat** | Tan felt. Flat, wide, level brim, seen obliquely so it carries **more brim in front of the face than behind** and the crown sits back over the occiput — the asymmetry is doing turn work as well as hat work. Crown pinched to a point, with a fore-and-aft crease. Brown leather band with a small buckle. The brim silhouette is the single strongest read at small size — draw it **wider than feels right**, about 1.4 head-widths. It must clear the brows: the brim was drawn once at a height that cut the eyes, which is the same defect as the player's toque covering the brows. **It still does not clear them.** Rendered at 8x across all four expressions on 2026-09-09, the brim's lower arc dips to y ≈ 64 over the face and the brows — y 51 to 67 — are entirely hidden, so this character plays three expression cues where the player plays five. The toque was fixed by raising its band; the brim cannot be raised, because the crown's ink is already at y 2 and nothing in this rig may sit above y = 0. It needs the brim's lower arc reshaped instead, which changes the single strongest identification cue this character has and is a measured job on its own, against the 0.92. Recorded, not done. | `felt`, `leather` |
...
| id | question | owner |
|---|---|---|
| ~~**OQ-ART-08**~~ | ~~Is the officer's gender presentation fixed, or a player choice?~~ **Answered 2026-09-14 by the PO: a player choice**, and it was a data change as this row promised: `presentation` is a slot with `feminine`, `masculine` and `neutral`, carried by face detail on the one artboard (`rig-contract.md` §3). The officer is not pinned and resolves to `neutral`. (PO's `OQ-LEVEL-3`.) | PO |
