...
**Re-proportioned 2026-09-14, after "the character looks goofy".** Measured on the default figure at
390 px before the change, the read came from six things at once, and each was changed rather than argued:

| measured before | why it read goofy | now |
|---|---|---|
| head ink 87 px tall, 121 px with the toque: 1 : 3.8 of the figure | a bobblehead; the canon unit is 70 | every head-pivot part drawn at **0.9** about the jaw (y 121), ink weight held at 6 px: head **78 px**, crown at y 44, neck length unchanged |
| toque a tall peaked cap, white fold, pom at the back | a Santa hat | a snug knit crown, a darker red ribbed cuff, the pom on top |
| eyes touching (0 px between inks), thick 3 px rings, no catchlight | goggles | 5.7 px apart, 2.5 px rings, a white catchlight in every eye, pupils looking forward |
| nose 12 px past the brow; lip and chin level with the brow | a muzzle | nose 8–9 px, lip and chin set 3–4 px behind the brow, cranium carried 4 px further back |
| neck 22 px of skin in a 34 px outer under a 93 px head | a stalk | **26 px in a 38 px outer** under an 84 px head: 0.45 of its width |
| straight capsule limbs, a closed ink cap at every elbow and knee, three quilt lines and a full-length highlight on each sleeve; mitts 60 px across; idle limbs dead straight | an action figure | limbs taper to each joint, the lower part's top cap is open, the sleeve lines are gone and the highlight is 8 px; hands at **0.85**; soft elbows and a 12 px stance at idle |

The officer and the guide took every one of the proportion changes — head scale, neck, taper, hands — so the
three still stand to one canon; only the toque and the sleeve lines are the parka's alone. The chin-to-shoulder
row above is improved, not met.
...
the page moved **0.13 MiB**. The old packer moved **5.97 MiB** under the same drift. The page's *shape* still
changes, because only its area is charged. The drift search above is no longer needed, but the rule in bold
still stands: read the page.
...
- ~~The faint rectangular hairlines around characters in the running game are older than this redraw. They show in
  ...
  and fails if any ring pixel is not the edge pixel beside it.
...
| ~~**OQ-ART-08**~~ | ~~Is the officer's gender presentation fixed, or a player choice?~~ **Answered 2026-09-14 by the PO: a player choice.** `presentation` is a slot with `feminine`, `masculine` and `neutral`, carried by brows, lashes, lip and jaw detail on the one artboard; no proportion differs (`rig-contract.md` §3). | PO (`OQ-LEVEL-3`) | the officer rig contract, task 1.11 |
