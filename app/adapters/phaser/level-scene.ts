...
/** How tall a placeholder parallax band is drawn, in design pixels. */
const PLACEHOLDER_BAND_HEIGHT = 230;
...
/** The player placeholder, in design pixels. Replaced by `ICharacterRenderer` (task 1.12). */
  ...
  #paintPois(): void {
    ...
    for (const poi of level.pois) {
      ...
      if (this.textures.exists(poi.artKey)) {
        this.add.image(poi.position.x, y, poi.artKey).setOrigin(0.5, 1).setDepth(DEPTH_ACTORS - 1);
        continue;
      }
